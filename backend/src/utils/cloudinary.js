...
const uploadOnCloudinaryWithBase64 = async (localFilePath) => {
  try {
    if (!localFilePath) return null;
    //upload the file on cloudinary
    ...
    fs.unlinkSync(`./public/temp/${localFilePath}`); // remove the locally saved temporary file as the upload operation got failed
    throw new ApiError(500, "Failed to upload one or more files.");
  }
};
